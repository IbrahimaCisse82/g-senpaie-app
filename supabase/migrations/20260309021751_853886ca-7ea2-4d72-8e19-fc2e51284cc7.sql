...
-- Drop all existing RESTRICTIVE policies and recreate as PERMISSIVE for all 7 tables
...
CREATE POLICY "Users can view own categories" ON public.convention_categories FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM conventions c WHERE c.id = convention_categories.convention_id AND c.user_id = auth.uid()));
CREATE POLICY "Users can insert own categories" ON public.convention_categories FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM conventions c WHERE c.id = convention_categories.convention_id AND c.user_id = auth.uid()));
...
-- ══ Recreate triggers (were dropped previously) ══
...
CREATE TRIGGER update_conventions_updated_at BEFORE UPDATE ON public.conventions
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_payroll_params_updated_at ON public.payroll_params;
CREATE TRIGGER update_payroll_params_updated_at BEFORE UPDATE ON public.payroll_params
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_payroll_history_updated_at ON public.payroll_history;
CREATE TRIGGER update_payroll_history_updated_at BEFORE UPDATE ON public.payroll_history
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
