...
export function StatCard({ icon, label, value, sub, color }: StatCardProps) {
  return (
    <div className={`bg-card border border-border rounded-lg p-5 flex-1 min-w-[155px] border-t-[3px] ${colorMap[color]}`}>
      <div className="text-xl mb-1.5">{icon}</div>
      <div className="text-muted-foreground text-[10px] uppercase tracking-wider mb-1">{label}</div>
      <div className={`text-lg font-extrabold ${colorMap[color].split(" ").pop()}`}>{value}</div>
      {sub && <div className="text-senpaie-dim text-[11px] mt-1">{sub}</div>}
    </div>
  );
}
